...
const PaginationComponent = ({
  ...
  const getItemsToRender = () => {
    if (totalPages <= MAX_ITEMS_TO_SHOW + 2) { // Include first and last page
      return middlePages;
    }
    if (currentPage >= 0 && currentPage < MAX_ITEMS_TO_SHOW) {
      return middlePages.slice(0, MAX_ITEMS_TO_SHOW);
    }
    if (currentPage >= MAX_ITEMS_TO_SHOW && currentPage < totalPages - MAX_ITEMS_TO_SHOW) {
  ...
  return (
    <Pagination className="h-8 pb-6">
      <PaginationContent>
        ...
        <PaginationItem
        className={clsx({'border border-slate-400 rounded': currentPage === firstPage})}
        >
        ...
        {...(getItemsToRender() ?? []).map((value) => (
          <PaginationItem
          className={clsx({'border border-slate-400 rounded': currentPage === value})}
          >
        ...
        {lastPage !== firstPage && <PaginationItem
          className={clsx({'border border-slate-400 rounded': currentPage === lastPage})}
        >
